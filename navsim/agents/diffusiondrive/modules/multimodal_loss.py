...
class LossComputer(nn.Module):
    ...
    def forward(self, poses_reg, poses_cls, targets, plan_anchor):
        ...
        #target_traj = target_traj.float().contiguous()
        #plan_anchor = plan_anchor.float().contiguous()

        # 显式切片并避免使用高阶索引
        #target_xy = target_traj.unsqueeze(1)[:, :, :, :2]  # (B, 1, T, 2)
        #anchor_xy = plan_anchor[:, :, :, :2]               # (B, K, T, 2)

        # 相减并求 L2 范数（等价于 torch.linalg.norm(..., dim=-1)）
        #diff = target_xy - anchor_xy
        #dist = torch.sqrt((diff ** 2).sum(dim=-1))  # shape: (B, K, T)
        dist = torch.linalg.norm(target_traj.unsqueeze(1)[...,:2] - plan_anchor, dim=-1)
