...
def transfuser_loss(
    ...
    if "trajectory_loss" in predictions:
        ...
        #targets["trajectory"] = targets["trajectory"].to(device).to(predictions["trajectory"].dtype)
...
def _agent_loss(
    ...
    #print(type(num_gt_instances))
    #if num_gt_instances > 0:
    #    num_gt_instances = num_gt_instances
    #else:
    #    num_gt_instances =num_gt_instances + torch.tensor(1,device="npu")
    num_gt_instances = num_gt_instances if num_gt_instances > 0 else num_gt_instances + 1
    #num_gt_instances = max(int(num_gt_instances), 1)
    #num_gt_instances = (num_gt_instances.item() if isinstance(num_gt_instances, torch.Tensor) else num_gt_instances)
    #num_gt_instances = max(num_gt_instances, 1)
    #num_gt_instances = torch.where(num_gt_instances > 0, num_gt_instances, num_gt_instances + 1)
